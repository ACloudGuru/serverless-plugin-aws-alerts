...
class ExternalStack {
  ...
  getExternalStackConfig() {
    return this.serverless.service.custom.alerts.externalStack || this.options['alerts-external-stack'] || '';
  }

  getExternalStackNameSuffix() {
    return (this.getExternalStackConfig() && this.getExternalStackConfig().nameSuffix) || '-alerts';
  }
  ...
  fixLambdaFunctionAndLogGroupReferences(parent, childKey, resource, preMergedResources) {
    // This does not check for circular references, but they shouldn't occur in CloudFormation properties
    if (Array.isArray(resource)) {
      for (let index = 0; index < resource.length; index++) {
        this.fixLambdaFunctionAndLogGroupReferences(resource, index, resource[index], preMergedResources);
      }
      ...
      for (const key in resource) {
        if (key === 'Ref' && typeof resource[key] === 'string') {
          ...
          if (!preMergedResources[refName]) {
            ...
            const mainResource = this.serverless.service.provider.compiledCloudFormationTemplate.Resources[refName];
            if (mainResource && mainResource.Properties && mainResource.Properties.FunctionName) {
              ...
              this.serverless.cli.log('Warning: Unresolved external alert stack reference: ' + refName);
            }
          }
        } else if (key === 'DependsOn' && typeof resource[key] === 'string' && parent && childKey) {
          // Found a dependency. See if it's unresolved.
          const refName = resource[key];
          if (!preMergedResources[refName]) {
            if (this.serverless.service.provider.compiledCloudFormationTemplate.Resources[refName]) {
              // It's a dependency on the main stack. We can drop it because it's already been deployed.
              delete parent[childKey];
            } else {
              this.serverless.cli.log('Warning: Unresolved external alert stack dependency: ' + refName);
            }
          }
        }
        this.fixLambdaFunctionAndLogGroupReferences(resource, key, resource[key], preMergedResources);
      }
    }
  }
  ...
  mergeQueuedResources() {
    ...
    for (let resource of this.queuedResources) {
      this.fixLambdaFunctionAndLogGroupReferences(null, null, resource, preMergedResources);
  ...
  afterDeployGlobal() {
    ...
    const externalStackName = this.provider.naming.getStackName() + this.getExternalStackNameSuffix();
  ...
  waitForExternalStack(externalStackName, operation) {
    let dots = 0;
    const readMore = () => {
      return this.describeExternalStack(externalStackName)
      .then(response => {
        if (!response) {
          // Stack does not exist
          if (dots) this.serverless.cli.consoleLog('');
          this.serverless.cli.log('External alert stack ' + externalStackName + ' removed successfully.');
          return;
        }
        ...
        if (state === 'in_progress') {
          ...
          if (dots) this.serverless.cli.consoleLog('');
          this.serverless.cli.log('External alert stack ' + externalStackName + ' ' + this.phrases[operation][state] + ' (' + response.StackStatus + ').');
          if (this.stackStatusCodes[response.StackStatus] === 'failure') {
            // The operation failed, so return an error to Serverless
            return Promise.reject(new Error('External alert stack ' + externalStackName + ' ' + this.phrases[operation][state] + ' (' + response.StackStatus + ')'));
          }
        }
      })
    }
  ...
  deployExternalStack(externalStackName) {
    ...
    const configResources = (externalStackConfig && externalStackConfig.resources) || {};
    const compiledCloudFormationTemplate = {
      "AWSTemplateFormatVersion": "2010-09-09",
      "Description": configResources.Description || "External AWS CloudFormation template for alerts",
      "Metadata": configResources.Metadata || undefined,
      "Parameters": Object.assign({}, this.refParameters, configResources.Parameters || {}),
      ...
      "Resources": Object.assign({}, configResources.Resources || {}, this.mergedResources),
    ...
    if (typeof externalStackConfig && externalStackConfig.stackTags === 'object') {
    ...
    const deployParameters = externalStackConfig && externalStackConfig.deployParameters || [];

    return this.describeExternalStack(externalStackName)
    .then(existingStack => {
      if (existingStack) {
        return this.updateExternalStack(externalStackName, compiledCloudFormationTemplate, deployParameters, stackTags);
      } else {
        return this.createExternalStack(externalStackName, compiledCloudFormationTemplate, deployParameters, stackTags);
      }
    });
  }
  ...
  uploadCloudFormationTemplate(compiledCloudFormationTemplate) {
    ...
    const compiledTemplateFileName = 'compiled-cloudformation-alerts-template.json';
    const params = {
    ...
    if (deploymentBucketObject) {
      params = this.setServersideEncryptionOptions(params, deploymentBucketObject);
    }
    ...
    .then(() => {
      ...
      const s3Endpoint = this.getS3EndpointForRegion(this.provider.getRegion());
  ...
  createExternalStack(externalStackName, compiledCloudFormationTemplate, deployParameters, stackTags) {
    this.serverless.cli.log('Creating external alert stack ' +  externalStackName + ' (' + Object.keys(this.mergedResources).length + ' resources configured)...');
  ...
  updateExternalStack(externalStackName, compiledCloudFormationTemplate, deployParameters, stackTags) {
    this.serverless.cli.log('Updating external alert stack ' +  externalStackName + ' (' + Object.keys(this.mergedResources).length + ' resources configured)...');
    ...
    .then(null, err => {
      if (err.message && err.message.match(/^No updates/)) {
        ...
        this.serverless.cli.log('External alert stack ' + externalStackName + ' has not changed.')
        return Promise.resolve();
      } else {
        return Promise.reject(err);
      }
  ...
  deleteExternalStack(externalStackName, becauseNoResources) {
    this.serverless.cli.log('Removing external alert stack ' + externalStackName + (becauseNoResources ? ' (no resources configured)...' : '...'));
    return this.provider.request(
      'CloudFormation',
      'deleteStack', {
        StackName: externalStackName,
